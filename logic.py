...
class Logic():
	...
	def get_soup(self, link):
		...
		r = self.session.get(link, headers=self.headers, cookies=self.cookies) # подсунули свой кук с согласием
...
if __name__ == '__main__':
	#тесты
	logic = Logic()
	categories = logic.get_categories_dict(logic.get_soup(logic.url))
	...
	#	pagination = logic.get_pagination_list(link)
	#	print(pagination)
		#for piece_of_cake in range(pagination):
		#	logic.get_cards(piece_of_cake)
	...
	print('метод: get_categories, тип:', type(logic.get_categories(soup)), logic.get_categories(soup))
	...
	print('метод: get_pagination_links (на примере одной категории), тип:', type(logic.get_pagination_links(categories_dict['Scenes'])), logic.get_pagination_links(categories_dict['Scenes'])) #список кусков-страниц одной категории
